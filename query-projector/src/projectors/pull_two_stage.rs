...
use {
    ...
    RelResult, Row, Rows, Schema, TypedIndex,
};
...
impl Projector for ScalarTwoStagePullProjector {
    fn project<'stmt, 's>(
        ...
        let results = if let Some(row) = rows.next()? {
            let entity: Entid = row.get_unwrap(0); // This will always be 0 and a ref.
...
impl TupleTwoStagePullProjector {
    ...
    fn collect_bindings<'a, 'stmt>(&self, row: &Row<'stmt>) -> Result<Vec<Binding>> {
...
impl RelTwoStagePullProjector {
    ...
    fn collect_bindings_into<'a, 'stmt, 'out>(
        &self,
        row: &Row<'stmt>,
        out: &mut Vec<Binding>,
    ) -> Result<()> {
